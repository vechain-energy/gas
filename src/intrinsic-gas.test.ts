...
describe('intrinsicGas(clause[])', () => {
    ...
    it('calculates contract creation speciality', () => {
        expect(intrinsicGas([{ to: null, data: '0x', value: '0x' }])).toEqual(TX_GAS + CLAUSE_GAS_CONTRACT_CREATION)
    })
    ...
    it('calculates zero bytes data gas', () => {
        ...
        expect(intrinsicGas([{ to: '0x', data: `0x${'00'.repeat(amountZeroBytes)}`, value: '0x' }])).toEqual(TX_GAS + CLAUSE_GAS + (amountZeroBytes * ZERO_BYTES_GAS))
    })

    it('calculates non-zero bytes data gas', () => {
        const amountNonZeroBytes = 4
        expect(intrinsicGas([{ to: '0x', data: `0x${'11'.repeat(amountNonZeroBytes)}`, value: '0x' }])).toEqual(TX_GAS + CLAUSE_GAS + (amountNonZeroBytes * NON_ZERO_BYTES_GAS))
    })
